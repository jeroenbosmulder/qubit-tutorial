...
function StepEvidenceRoom({ openExtra }) {
  return (
    <div>
      ...
      <p>
        To hold that evidence in your own hands, five playgrounds are waiting. They are heavier than the ten steps — take them in any dose, or not at all.
      </p>
      {[
      ...
      <Notice>
        Each playground stands on its own, but they read best in order: first the ruler between coins (A), then its price in flips (B), then the ruler between beliefs (C), then the machinery under the hood (D), and finally the state's own axes (E) — where the quantum words <em>eigen-state</em> and <em>eigenvalue</em> turn out to be old acquaintances from data analysis. The navigation at the top always brings you back to the main tutorial.
      </Notice>
    </div>
  );
}
